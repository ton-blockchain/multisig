...
export const checkJettonMinter = async (
    ...
    const result = await sendToIndex('account', {address: addressToString(jettonMinterAddress)}, isTestnet);
    ...
    const provider = new MyNetworkProvider(jettonMinterAddress.address, isTestnet);

    const jettonMinterContract: JettonMinter = JettonMinter.createFromAddress(jettonMinterAddress.address);
    ...
    let nextAdminAddress = undefined;
